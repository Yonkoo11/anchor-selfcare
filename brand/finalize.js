...
const CREAM='#FAF8F5', SAGE='#7D8B7A', MOSS='#6B7B6A';
const P = '../public/';

// the locked mark. solid head, wide crossbar, shallow cradle, no barbs.
const mark = (sw=34) => `<g fill="none" stroke="${CREAM}" stroke-width="${sw}"
...
const square = tile(), rounded = tile({rx:96}), chunky = tile({rx:96, sw:44});

(async () => {
  // --- X profile picture, full bleed, X applies the circular crop itself
  ...
  // --- app icon set, replacing the ones that said SC
  fs.writeFileSync(P+'icon.svg', rounded);
  fs.writeFileSync(P+'apple-touch-icon.svg', rounded);
  fs.writeFileSync(P+'favicon-32x32.svg', chunky);
  fs.writeFileSync(P+'favicon-16x16.svg', chunky);
  ...
  // --- proof: every target size, actual pixels, circular where X crops
  ...
  const comp = [{input:'proof-400.png',left:20,top:20},{input:'proof-128.png',left:450,top:20}];
