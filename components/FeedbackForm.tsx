...
export function FeedbackForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // For now, use mailto as a privacy-first fallback
    // Can be swapped for Formspree, custom API, etc.
    const subject = encodeURIComponent(`Self-Care Guide Feedback: ${type}`)
    const body = encodeURIComponent(
      `Type: ${type}\n\nMessage:\n${message || '(No additional message)'}\n\n---\nSent from Self-Care Guide feedback form`
    )

    // Open mailto (works offline, no data stored anywhere)
    window.location.href = `mailto:feedback@selfcare-guide.app?subject=${subject}&body=${body}`
